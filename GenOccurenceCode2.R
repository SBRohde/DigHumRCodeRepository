...
rokossovsks_v <- grep("рокоссовск", corpus_word_v)
rokossovsks_v
r_count_v <- rep(NA, times = length(n_time_v))
r_count_v[rokossovsks_v] <- 1
plot(
  r_count_v,
  main = "Dispersion Plot of 'Рокоссовск*' in Text Corpus",
  xlab = "Corpus Time",
  ylab = "Рокоссовск*",
  type = "h",
  ylim = c(0, 1), yaxt = 'n'
